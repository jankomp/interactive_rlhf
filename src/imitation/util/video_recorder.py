...
class VideoRecorder:
    ...
    def __init__(
        ...
        Args:
            ...
            resize_resolution (Optional[tuple]): Resolution to resize frames before saving (width, height).
            disable_logger (bool): Whether to disable logging messages.
    ...
    def capture_frame(self):
        ...
        if frame is None:
            ...
            if self.video_writer is None:
                ...
                self.video_writer = cv2.VideoWriter(self.path, fourcc, self.frames_per_sec, (self.frame_width, self.frame_height))

            # Convert RGB (OpenAI Gym standard) to BGR (OpenCV standard)
...
# Example usage of resetting the video recorder:
def reset_video_recorder(env, recorder):
    if recorder is not None:
        recorder.close()
    return VideoRecorder(env)
